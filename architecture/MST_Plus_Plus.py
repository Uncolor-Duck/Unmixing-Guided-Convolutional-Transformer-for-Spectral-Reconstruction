...
class MST(nn.Module):
    def __init__(self, in_dim=48, out_dim=48, dim=48, stage=48, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=48, n_feat=48, stage=1):
        super(MST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        modules_body = [MST(dim=out_channels, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
